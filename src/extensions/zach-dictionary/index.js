...
class ZachDictionaryBlocks {
    ...
    static get STATE_KEY () {
        return 'Zach.Dictionary';
    }

    _getState (target) {
        let state = target.getCustomState(ZachDictionaryBlocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(ZachDictionaryBlocks.DEFAULT_DICTIONARY_STATE);
            target.setCustomState(ZachDictionaryBlocks.STATE_KEY, state);
        }
        return state;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const state = sourceTarget.getCustomState(ZachDictionaryBlocks.STATE_KEY);
            if (state) {
                newTarget.setCustomState(ZachDictionaryBlocks.STATE_KEY, Clone.simple(state));
            }
        }
    }

    static get DEFAULT_DICTIONARY_STATE () {
        return {
            currentInstrument: 0
        };
    }
...
module.exports = ZachDictionaryBlocks;
